      ...
      content: (
        <>
          ...
          <ul>
            ...
            <li><strong>{isKo ? '최신 뉴스' : 'Latest News'}</strong>: {isKo ? 'AI·DX 분야의 최신 뉴스와 아티클 공유' : 'Sharing latest news and articles in AI/DX'}</li>
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>{isKo ? 'AI·DX 학습에 도움이 되는 외부 리소스를 소개합니다.' : 'External resources helpful for AI/DX learning.'}</p>
          ...
          <table className="guide-table">
            ...
            <tbody>
              ...
              <tr><td><strong>DeepLearning.AI</strong></td><td>{isKo ? 'AI 전문 과정, ChatGPT 활용법' : 'AI specialized courses, ChatGPT usage'}</td><td>{isKo ? 'AI 엔지니어 지망생' : 'Aspiring AI engineers'}</td></tr>
        ...
        <>
          ...
          <table className="guide-table">
            ...
            <tbody>
              ...
              <tr><td><strong>{isKo ? 'AI EXPO Korea' : 'AI EXPO Korea'}</strong></td><td>{isKo ? '매년 3~4월' : 'March~April annually'}</td><td>{isKo ? '국내 최대 AI 전시회' : "Korea's largest AI exhibition"}</td></tr>
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <h3>{isKo ? 'Q. AI를 배우려면 프로그래밍을 알아야 하나요?' : 'Q. Do I need programming to learn AI?'}</h3>
          <p>{isKo
          ...
          <p>{isKo
            ? 'A. 네, 모든 학습 콘텐츠는 무료로 제공됩니다. 회원가입 후 북마크, 학습 진행률 추적 등 추가 기능을 이용할 수 있습니다.'
          ...
          <TipBox type="important">
            <p>{isKo
              ? '추가 질문이 있으시면 각 학습 페이지의 댓글 기능을 이용해 주세요. 커뮤니티 멤버와 운영진이 답변해 드립니다.'
      ...
      content: (
        <>
          ...
          <ol>
            ...
            <li><strong>{isKo ? '건설적 토론' : 'Constructive Discussion'}</strong>: {isKo ? '비판은 건설적으로, 근거를 제시하고, 대안을 함께 제안해 주세요.' : 'Criticize constructively with evidence and suggest alternatives.'}</li>
            <li><strong>{isKo ? '출처 명시' : 'Cite Sources'}</strong>: {isKo ? '외부 자료를 인용할 때는 반드시 출처를 밝혀 주세요.' : 'Always cite sources when referencing external materials.'}</li>
