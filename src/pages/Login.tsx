          ...
          {accountBlock && (
            <div className="auth-error" style={{ marginBottom: '16px' }}>
              {accountBlock.status === 'banned' && (
                <>계정이 차단되었습니다. {accountBlock.reason && <>사유: {accountBlock.reason}</>}</>
              )}
              {accountBlock.status === 'deleted' && <>탈퇴 처리된 계정입니다.</>}
              <button onClick={clearAccountBlock} style={{ display: 'block', marginTop: '8px', background: 'none', border: 'none', textDecoration: 'underline', cursor: 'pointer', color: 'inherit', fontSize: '12px', padding: 0 }}>
                닫기
              </button>
            </div>
          )}
